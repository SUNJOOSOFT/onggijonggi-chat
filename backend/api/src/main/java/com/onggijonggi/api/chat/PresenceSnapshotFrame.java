 ...
 *               스스로를 목록에 넣을 방법이 없다 — 프레임이 싣는 userId는 app_user.id인데 그 값은
 *               브라우저 세션에 없다(프론트 types/next-auth.d.ts).
 *
 ...
 */
public record PresenceSnapshotFrame(UUID sessionId, List<UUID> participants) implements WsFrame {
}
